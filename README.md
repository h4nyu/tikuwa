...
一覧の上部にある「全選択」ボタンでチェックボックスをまとめてオンにでき、「全削除」
ボタンでチェックした記録をまとめて削除できる(確認ダイアログあり)。その下のカテゴリ選択欄と
「カテゴリを一括変更」ボタンでは、チェックした記録のカテゴリをまとめて選んだ値に変更できる。
CSV書き出し・一括削除・カテゴリ一括変更のいずれもこのチェックボックスの選択状態を使い、
実行後は選択が解除される。
