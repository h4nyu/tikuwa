...
記録の削除は一覧上のボタンではなく、編集フォームの「更新する」ボタンの隣にある「削除」ボタンから行う
(確認ダイアログあり)。

各記録の左側にチェックボックスがあり、チェックした記録だけが「CSVで書き出す(Excelで開けます)」の対象になる
(検索・カテゴリチップで一時的に隠れても選択状態は保たれる)。「商品名」「追跡番号」「運送会社」
「カテゴリ」「総国際追跡番号」「記録日時」の6列のCSVファイルとしてダウンロードでき、書き出した記録の
...
「全削除」でまとめてオフにできる。その下のカテゴリ選択欄と「カテゴリを一括変更」ボタンでは、
チェックした記録のカテゴリをまとめて選んだ値に変更できる。CSV書き出し・カテゴリ一括変更は
