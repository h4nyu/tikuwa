...
「CSVで書き出す(Excelで開けます)」ボタンをタップすると、カテゴリが「上海到着」の記録
(検索欄の絞り込みも反映)の件数が表示され、複数の記録をまとめて発送する1つの国際便の
「総国際追跡番号」の入力を求められる。入力して確定すると「商品名」「追跡番号」「運送会社」
「総国際追跡番号」「記録日時」の5列のCSVファイルがダウンロードされ、対象の記録は
カテゴリが「国際発送」に、総国際追跡番号が入力した値に、まとめて自動更新される
(次回の書き出しで同じ記録が重複しないようにするため)。
