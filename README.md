...
カテゴリ(任意)・国際追跡番号(任意)を登録できる(商品登録と同じ「+」ボタンだが、物流タブ表示中は
こちらのフォームが開く)。カテゴリは「発注済み」「上海到着」「国際発送」「到着済み」の4択から選ぶ。
商品の在庫数とは連動しない、単純な記録用の一覧(新しい順)。画面最上部の検索欄で商品名・追跡番号・
運送会社・カテゴリ・国際追跡番号の部分一致検索ができ、その下のカテゴリチップ(在庫一覧のカテゴリ
フィルタと同じ見た目)と組み合わせて絞り込める。各記録の「編集」リンクから同じフォームを開いて
商品名・追跡番号・運送会社・カテゴリ・国際追跡番号を後から修正できる。カテゴリを設定すると記録
一覧にバッジとして表示され、国際追跡番号を設定すると一覧の商品名の並びに「国際:番号」の形式で表示される。
記録の削除は一覧上のボタンではなく、編集フォームの「更新」ボタンの隣にある「削除」ボタンから行う
(確認ダイアログあり)。

カテゴリが「国際発送」かつ国際追跡番号が設定されている記録は、同じ番号を持つ記録どうしが
親子ツリーとしてまとめて表示される(親ノードに「国際追跡番号: 番号」と件数、その下に該当する
記録が並ぶ)。親ノードの▼/▶ボタンで展開・折りたたみができる。それ以外のカテゴリの記録や
国際追跡番号が未設定の記録は、従来通りフラットな一覧として表示される。

各記録の左側にチェックボックスがあり、チェックした記録だけが「CSVで書き出す(Excelで開けます)」の対象になる
(検索・カテゴリチップで一時的に隠れても選択状態は保たれる)。「商品名」「追跡番号」「運送会社」
「カテゴリ」「国際追跡番号」「記録日時」の6列のCSVファイルとしてダウンロードでき、書き出した記録の
うちカテゴリが「上海到着」だったものは国内転送が済んだとみなし、自動的に「国際発送」に更新される
(書き出し後はチェックが解除される)。Excelでの文字化けを防ぐためUTF-8 BOM付きで出力している。

一覧の上部にある「全選択」「全削除」ボタンは記録そのものではなくチェックボックスの状態だけを操作する
(「全削除」を押しても記録は削除されない)。「全選択」で表示中のチェックボックスをまとめてオンにでき、
「全削除」でまとめてオフにできる。その下の「選択した項目を統一編集」ボタンでは、チェックした記録の
運送会社・カテゴリ・国際追跡番号をまとめて同じ値に変更できるモーダルが開く(空欄のままの項目は
変更されない)。CSV書き出し・統一編集はこのチェックボックスの選択状態を使い、実行後は選択が解除される。
